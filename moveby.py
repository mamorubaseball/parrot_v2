...
def main():
    ...
    df = pandas.read_csv('CSV/lat_lon_10data.csv')
    for index,row in df.iterrows():
        l = [row.lat,row.lon,row.h]
        x,y,z,direction = calcurate(drone,l)
        drone(moveBy(0, 0, 0, direction)
            >> FlyingStateChanged(state="hovering", _timeout=3)).wait().success()
        time.sleep(1)
        drone(moveBy(y, x, z, 0)
...
def main_command():
    ...
    df = pandas.read_csv('CSV/move_by_command.csv')
    for index,row in df.iterrows():
        drone(moveBy(0, 0, 0, row.diff_rad)
            >> FlyingStateChanged(state="hovering", _timeout=3)).wait().success()
        time.sleep(1)
        drone(moveBy(row.distance, 0, row.diff_h, 0)
            >> FlyingStateChanged(state="hovering", _timeout=3)).wait().success()
    drone_gps=drone.get_state(PositionChanged)
    d=get_distance(goal[0],goal[1],drone_gps['latitude'],drone_gps['longitude'],8)
    print('===============ゴールとの差：'+d+'==========================')
